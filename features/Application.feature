...
#    Then the submit button becomes clickable
  ...
  Scenario Outline: the user should be able to fill out the form
    Given the user is on the landing page
    When the user navigates to Loan purpose page
    And the user chooses loan purpose <loan purpose>
    And the user chooses education level <education level>

    Examples:
      | loan purpose            | education level |
      | Debt consolidation      | High school     |
      | Credit card refinancing | Associate       |
